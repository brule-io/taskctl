...
def main():
    ...
    parser.add_argument('--kind', choices=['native','jvm'], default='native')
    args = parser.parse_args()
    system = target(); windows = system.startswith('windows')
    metadata = json.loads((ROOT/f'build/distributions/{args.kind}-{system}.json').read_text(encoding='utf-8'))
    ...
    with tempfile.TemporaryDirectory(prefix='bootstrap ', dir=output) as temporary:
        ...
        for arguments in (['--version'],['-V'],['version']):
            assert run(arguments,extra={'TASKCTL_OFFLINE':'1'}).stdout == f'taskctl {metadata["version"]}\n'
        version=json.loads(run(['version','--format','json']).stdout)
        assert version==dict(api='taskctl.cli/alpha1',command='version',result=dict(tool='taskctl',version=metadata['version']))
        assert not (work/'cache').exists(), 'Version reporting acquired or mutated a cache'
        ...
        assert info['implementation']==args.kind and info['build']['implementation']==args.kind
        ...
        executable=cached/('taskctl.exe' if windows else 'taskctl')
        jar=next((cached/'lib').glob('*.jar')) if args.kind=='jvm' else executable
        original=jar.read_bytes(); jar.write_bytes(original+b'tampered')
        ...
        (cached/'lib').mkdir(exist_ok=True)
